...
const SubmissionHistory = ({ problemId }) => {
  ...
  useEffect(() => {
    const fetchSubmissions = async () => {
      try {
        setLoading(true);
        const response = await axiosClient.get(`/api/v1/problems/submittedProblem/${problemId}`);
        ...
        <>
          <div className="overflow-x-auto">
            <table className="table w-full">
              <thead>
                <tr className="bg-gray-100 text-gray-900 font-bold text-base">
                  <th>#</th>
                  <th>Language</th>
                  <th>Status</th>
                  <th>Runtime</th>
              ...
              <tbody>
                {submissions.map((sub, index) => (
                  <tr key={sub._id}>
                    <td>{index + 1}</td>
                    <td className="font-mono text-gray-900">{sub.language}</td>
                    <td>
                      <span className={`badge ${getStatusColor(sub.status)} text-white`}>
                        {sub.status.charAt(0).toUpperCase() + sub.status.slice(1)}
                      </span>
                    </td>

                    <td className="font-mono text-gray-900">{sub.runtime}sec</td>
                    <td className="font-mono text-gray-900">{formatMemory(sub.memory)}</td>
                    <td className="font-mono text-gray-900">{sub.testCasesPassed}/{sub.totalTestCases}</td>
      ...
      {selectedSubmission && (
        <div className="modal modal-open">
          <div className="modal-box w-11/12 max-w-5xl bg-white text-gray-900">
            ...
            <pre className="p-4 bg-gray-100 text-gray-900 rounded overflow-x-auto border border-gray-300">
              <code>{selectedSubmission.solutionCode}</code>
            </pre>
